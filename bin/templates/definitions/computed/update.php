		...
		<div class="material">
			<form method="POST" action="">
				<div class="frm-ctrl-outer">
					<input type="text" name="app" placeholder="" id="appid" class="frm-ctrl" value="<?= _q(_e($computed->app)) ?>">
				...
				<div class="frm-ctrl-outer">
					<select type="text" name="source" placeholder="" id="source" class="frm-ctrl">
						...
						<option value="<?= _q(_e($setting->_id)) ?>" <?= $computed->source->_id == $setting->_id? 'selected' : '' ?>><?= _q(_e($setting->caption)) ?></option>
						<?php endforeach ?>
					</select>
				...
				<div class="frm-ctrl-outer">
					<select type="text" name="target" placeholder="" id="target" class="frm-ctrl">
						<?php foreach (db()->table('definitions\setting')->getAll()->all() as $setting): ?>
						<option value="<?= _q(_e($setting->_id)) ?>" <?= $computed->target->_id == $setting->_id? 'selected' : '' ?>><?= _q(_e($setting->caption)) ?></option>
						<?php endforeach ?>
					</select>
				...
				<label class="frm-lbl" for="hidden">
					<input type="checkbox" name="hidden" placeholder="" id="hidden" class="frm-ctrl"  <?= $computed->hidden? 'checked' : '' ?>>
